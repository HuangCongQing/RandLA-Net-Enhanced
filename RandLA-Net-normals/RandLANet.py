...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            self.inputs['sub_normals'] = flat_inputs[4 * num_layers:5 * num_layers]
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            f_encoder_i = self.dilated_res_block(feature, inputs['xyz'][i], inputs['neigh_idx'][i], d_out[i],
                                                 'Encoder_layer_' + str(i), is_training, inputs['sub_normals'][i])
    ...
    def dilated_res_block(self, feature, xyz, neigh_idx, d_out, name, is_training, sub_normals):
        f_pc = helper_tf_util.conv2d(feature, d_out // 2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_pc = self.building_block(xyz, f_pc, neigh_idx, d_out, name + 'LFA', is_training, sub_normals)
    ...
    def building_block(self, xyz, feature, neigh_idx, d_out, name, is_training, sub_normals):
        d_in = feature.get_shape()[-1].value
        f_xyz = self.relative_pos_encoding(xyz, neigh_idx, sub_normals)
        f_xyz = helper_tf_util.conv2d(f_xyz, d_in, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_neighbours = self.gather_neighbour(tf.squeeze(feature, axis=2), neigh_idx)
        f_concat = tf.concat([f_neighbours, f_xyz], axis=-1)
        f_pc_agg = self.att_pooling(f_concat, d_out // 2, name + 'att_pooling_1', is_training, feature)

        f_xyz = helper_tf_util.conv2d(f_xyz, d_out // 2, [1, 1], name + 'mlp2', [1, 1], 'VALID', True, is_training)
        f_neighbours = self.gather_neighbour(tf.squeeze(f_pc_agg, axis=2), neigh_idx)
        f_concat = tf.concat([f_neighbours, f_xyz], axis=-1)
        f_pc_agg = self.att_pooling(f_concat, d_out, name + 'att_pooling_2', is_training, feature)
        return f_pc_agg

    def relative_pos_encoding(self, xyz, neigh_idx, sub_normals):
        neighbor_xyz = self.gather_neighbour(xyz, neigh_idx)
        
        neighbor_normals = self.gather_neighbour(sub_normals, neigh_idx)
        
        xyz_tile = tf.tile(tf.expand_dims(xyz, axis=2), [1, 1, tf.shape(neigh_idx)[-1], 1])
        relative_xyz = xyz_tile - neighbor_xyz
        relative_dis = tf.sqrt(tf.reduce_sum(tf.square(relative_xyz), axis=-1, keepdims=True))
        
        # relative_feature = tf.concat([relative_dis, relative_xyz, xyz_tile, neighbor_xyz, neighbor_normals], axis=-1)
        relative_feature = tf.concat([relative_xyz, xyz_tile, neighbor_normals], axis=-1)
    ...
    @staticmethod
    def att_pooling(feature_set, d_out, name, is_training, feature):
