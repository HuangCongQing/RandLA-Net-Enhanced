...
class SemanticKITTI:
        ...
        def spatially_regular_gen():
            ...
            for i in range(num_per_epoch):
                if split != 'test':
                    ...
                    pc, tree, labels, normals = self.get_data(pc_path)

                    # crop a small point cloud
                    pick_idx = np.random.choice(len(pc), 1)

                    selected_pc, selected_labels, selected_idx, selected_normals = self.crop_pc(pc, labels, tree, pick_idx, normals)
                ...
                if True:
                    yield (selected_pc.astype(np.float32),
                           ...
                           selected_normals.astype(np.float32))

        gen_func = spatially_regular_gen
        gen_types = (tf.float32, tf.int32, tf.int32, tf.int32, tf.float32)
        gen_shapes = ([None, 3], [None], [None], [None], [None, 3])
    ...
    def get_data(self, file_path):
        ...
        normal_path = join(self.dataset_path, seq_id, 'normals', frame_id + '.npy')
        ...
        return points, search_tree, labels, normals

    @staticmethod
    def crop_pc(points, labels, search_tree, pick_idx, normals):
        ...
        select_normals = normals[select_idx]

        return select_points, select_labels, select_idx, select_normals

    @staticmethod
    def get_tf_mapping2():

        def tf_map(batch_pc, batch_label, batch_pc_idx, batch_cloud_idx, batch_normals):
            features = batch_pc
            input_points = []
            input_neighbors = []
            input_pools = []
            ...
            for i in range(cfg.num_layers):
                ...
                sub_normals = batch_normals[:, :tf.shape(batch_pc)[1] // cfg.sub_sampling_ratio[i], :]
